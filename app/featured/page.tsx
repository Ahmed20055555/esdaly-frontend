...
import { FiShoppingCart, FiHeart, FiStar, FiChevronRight, FiChevronLeft } from "react-icons/fi";
...
export default function FeaturedPage() {
  ...
  const fetchFeatured = async () => {
    try {
      setLoading(true);
      const res = await productsAPI.getFeatured({ page: currentPage, limit });
      if (res.success) {
        setProducts(
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-10">
        <Breadcrumbs items={[{ label: "الرئيسية", href: "/" }, { label: "منتجات مميزة" }]} />
        <div className="flex items-center gap-2 mt-2 mb-6">
          <FiStar className="w-8 h-8 text-amber-500 fill-amber-500" />
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900">منتجات مميزة</h1>
        </div>
        <p className="text-gray-600 mb-8">تشكيلة مختارة من أفضل منتجاتنا</p>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl shadow-xl p-12 text-center">
            <h2 className="text-xl font-bold text-gray-900 mb-4">لا توجد منتجات مميزة حالياً</h2>
            <p className="text-gray-600 mb-6">تصفحي كل المنتجات من الرئيسية</p>
              ...
              {products.map((item) => (
                ...
                >
                  <Link href={`/product/${item.id}`} className="relative aspect-square bg-gray-100 block">
                    ...
                    <span className="absolute top-2 right-2 bg-amber-500 text-white px-2 py-0.5 rounded text-xs font-medium flex items-center gap-1">
                      <FiStar className="w-3 h-3 fill-current" /> مميز
                    </span>
                  </Link>
